<script lang="ts">
    ...
    const getIsPinned = async (table: Tables, link: string): Promise<boolean> => {
...
>
    <p on:click={() => { isShowModalPostDetail = true }} class="!text-secondary !text-sm relative z-0 w-full cursor-pointer pb-4 h-full" style="word-break: break-all; overflow-wrap: break-word;">{truncateText(menfess.new_msg ? menfess.new_msg : menfess.message, 35)}</p>
    <div id="bottom" class="w-full flex justify-between items-center">
        <div id="stats" class="flex gap-2">
            <div id="comments" class="flex items-center gap-1">
                ...
                <span>{!menfess.num_comments ? "0" : menfess.num_comments}</span>
            </div>
            <div id="reactions" class="flex items-center gap-1">
                <i class="fa-solid fa-icons"></i>
                <span>{menfess.num_reactions}</span>
            </div>
        </div>
        <div class="relative">
