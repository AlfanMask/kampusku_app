...
export async function GET({ url }) {
    ...
    // get last 25 friends data
    let last20FriendData: Array<FessFriends> = await mysqlconn.query(`SELECT id, message, link, deleted, pinned_at, is_edited, num_comments, num_reactions, created_at, new_msg, 'friends' AS table_name FROM friends WHERE user_id = '${userId}' AND deleted = 0 ORDER BY id DESC LIMIT 25`).then(([rows, fields]) => {
        return rows as Array<FessFriends>;
    })
